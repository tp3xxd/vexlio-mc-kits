...
public class ItemStackBuilder {
   ...
   public ItemStackBuilder addEnchantment(String var1, int var2) {
      if (VexlioKits.getInstance().AdvancedEnchantmentsEnabled && AEAPI.isAnEnchantment(var1)) {
   ...
   public void setPotionEffect(PotionType var1, boolean var2, boolean var3) {
      ...
      try {
         ...
         Bukkit.getConsoleSender().sendMessage(ChatColor.RED + "[VexlioKits] The used format for potions does not work on this minecraft version, please try other format which is POTION:DURABILITY");
      }
   ...
   public ItemStackBuilder tagUnbreakable() {
      try {
         if (getItemSpigot != null) {
            ...
            Bukkit.getConsoleSender().sendMessage(ChatColor.RED + "[VexlioKits] The unbreakable tag is not useable for this minecraft server version!");
         }
