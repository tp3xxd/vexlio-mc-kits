...
public class PlayingMap extends Map {
   ...
   public PlayingMap(VexlioKits var1, String var2, List<Location> var3, List<Cuboid> var4, boolean var5, HashMap<Location, Integer> var6) {
   ...
   public void removePlayers() {
      if (this.plugin.bungeeMode != null) {
         ...
         if (var1 > 0) {
            for(Player var7 : Utils.getPlayers(this.plugin.players)) {
               if (PlayerDataManager.get(var7).getMap().name.equals(this.name)) {
                  if (!this.plugin.isInTournament(var7) && !this.plugin.isInChallenge(var7)) {
                     ...
                     var7.performCommand("vexliokits join " + this.plugin.playingMaps.values().iterator().next());
                  } else {
            ...
            ArrayList<Player> var6 = new ArrayList<>();
            ...
            for(Player var10 : var6) {
               var10.performCommand("vexliokits leave");
            }
         }
